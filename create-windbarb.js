...
pointRadius = 3, 
fillColor = '#2B85C7', 
strokeColorCircle = '#2B85C7', 
barbSpaceing = 5, 
barbHeight = 15, 
strokeColorPath = '#2B85C7',
flagFillColor = '#2B85C7',
strokeLength = 15,
forceDir = false;

function createPoint(deg, speed, mirrorVel) {
    var svg, w,h, sw, r, fc, sc;
    sw = strokeWidth, r = pointRadius, fc = fillColor, sc = strokeColorCircle;
    w = h = 2*sw+2*r;
	var b = createShadow(deg, speed, mirrorVel);
    return b;
}

function createBarbs(speed) {
...
function setIconStyles(img, name, a) {
    var sw,r,o;
    var size = {x: barbHeight + strokeLength, y: barbSpaceing + strokeWidth},//L.point(options[name === 'shadow' ? 'shadowSize' : 'iconSize']),
        anchor;
    ...
    if (name === 'shadow') {
        ...
        var x = w/2;
        var y = h/2;
        anchor = {x: barbHeight + strokeLength, y: barbSpaceing + strokeWidth};//L.point([x,y]);
    }
...
function createShadow(deg, speed, mirrorVel) {
    var d,s,b;
    d = deg+90,
    s = speed,
    b= createBarbs(s);
    ...
    setIconStyles(div, 'shadow', anchor);
    return div;
}